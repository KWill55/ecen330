...
#define LEFT_THIRD_SCREEN (LCD_W/3.0)
#define RIGHT_THIRD_SCREEN (2*(LCD_W/3.0))

#define LEFT_BASE (1.0/6 * LCD_W)
#define MIDDLE_BASE (1.0/HALF * LCD_W)
#define RIGHT_BASE (5.0/6 * LCD_W)
...
//this function finds and assigns the closest base to fire our player missiles from 
void assign_base(missile_t *missile, coord_t x_dest){
    
    //if x_dest is in left third of screen, assign left base 
    if (x_dest < LEFT_THIRD_SCREEN){
        missile->x_origin = LEFT_BASE;
    }
    //if x_dest is in middle third of screen, assign middle base 
    if ( (x_dest >= LEFT_THIRD_SCREEN) && (x_dest <= RIGHT_THIRD_SCREEN) ){
        missile->x_origin = MIDDLE_BASE;
    }
    //if x_dest is in right third of screen, assign right base 
    if (x_dest > RIGHT_THIRD_SCREEN){
...
void missile_init_player(missile_t *missile, coord_t x_dest, coord_t y_dest){
    ...
    assign_base(missile, x_dest); //this function sets x_origin to the correct base 
	missile->y_origin = LCD_H;
    // x,y destination is parameter (cursor position)
...
void missile_tick(missile_t *missile){
    ...
    switch (missile->currentState) {
        ...
        case MISSILE_STATE_MOVING:
            ...
            //if missile hits the ground, transition to impacted
                ...
                //if its a plane missile, transition to impacted
    ...
    switch (missile->currentState) {
        ...
        case MISSILE_STATE_MOVING:
            ...
            //move the missile if the missile is a plane missile
            else if (missile->type == MISSILE_TYPE_PLANE){
                missile->length += CONFIG_ENEMY_MISSILE_DISTANCE_PER_TICK;
        ...
        case MISSILE_STATE_EXP_GROWING:
            ...
            //draw plane missile explosion growing 
            else if (missile->type == MISSILE_TYPE_PLANE){
        ...
        case MISSILE_STATE_EXP_SHRINKING:
            ...
            //draw plane explosion shrkinking
            else if (missile->type == MISSILE_TYPE_PLANE){
                lcd_fillCircle(missile->x_current, missile->y_current, missile->radius, CONFIG_COLOR_PLANE_MISSILE);
